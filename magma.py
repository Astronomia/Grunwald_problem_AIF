...
from pygments.token import Comment, Operator, Keyword, Name, String, Number, Punctuation, Whitespace, Text, Generic, Error
...
class Magma(RegexLexer):
	...
	keywords = """ _ by default do is select then to where end until catch elif else when case for function if intrinsic procedure vrepeat try while """
	operators = """ adj and cat cmpeq cmpne diff div eq ge gt in join le lt meet mod ne notadj notin notsubset or sdiff subset xor not """
	extra_keywords = """ assert assert2 assert3 break clear continue declare delete error error if eval exit forward fprintf freeze iload import load local print printf quit random read readi require requirege requirerange restore return save time vprint vprintf vtime assigned exists forall """
	constructors = """ car case cop elt ext func hom ideal lideal map ncl pmap proc quo rec recformat rideal sub """
	block_open = """ try catch case when then else do repeat function procedure intrinsic """
	...
	tokens = {
		...
		'intrinsics': [
			(r'''(?sx)
				...
				(-> \s* (?: \S[^,]*? (?: \s* , \s* \S[^,]*?)* \s*))
				(\{)
			''', bygroups(Keyword, Whitespace, Name.Function, Whitespace, using(this), using(this), Comment), 'comment-intrinsic'),
		],
